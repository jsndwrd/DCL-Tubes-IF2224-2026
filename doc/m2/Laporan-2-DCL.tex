...
\begin{tikzpicture}[
    ...
    proc/.style={rectangle, draw, fill=yellow!12, rounded corners=2pt,
                 minimum width=3cm, minimum height=0.8cm},
    output/.style={rectangle, draw, fill=keyword!10, rounded corners=2pt,
                   minimum width=3cm, minimum height=0.8cm},
    arr/.style={-{Stealth[length=2.5mm]}, thick}
]
    \node[src]                         (src)  {\texttt{source.txt}};
    \node[proc, right=1.5cm of src]    (lex)  {Lexer (DFA)};
    \node[src,  right=1.5cm of lex]    (tok)  {Token Stream};
    \node[proc, below=1.4cm of tok]    (par)  {Parser (RD)};
    \node[output, left=1.5cm of par]   (tree) {Parse Tree};
...
menjadi dua file: \texttt{src/header/parser.hpp} mendeklarasikan struct
...
\texttt{<variable>}               & \texttt{ident (<component-variable>)*} \\
\texttt{<component-variable>}     & \texttt{(lbrack <index-list> rbrack) | (period ident)} \\
...
\begin{tikzpicture}[
    every node/.style={font=\small},
    dec/.style={diamond, draw, fill=yellow!12, aspect=2.6, inner sep=2pt, minimum width=3.6cm, minimum height=1.1cm},
    proc/.style={rectangle, draw, fill=codebg, rounded corners=2pt, minimum width=3.2cm, minimum height=0.8cm},
    arr/.style={-{Stealth[length=2.5mm]}, thick},
    lbl/.style={font=\scriptsize, fill=white, inner sep=1pt}
]
    \node[dec]                                   (d1)     {peek = ident?};
    \node[proc, right=2.5cm of d1]               (kw)     {keyword dispatch};
    \node[proc, below=1.4cm of d1]               (scan)   {scan \texttt{[\dots]} / \texttt{.ident}};
    \node[dec,  below=1.4cm of scan]             (d2)     {next = becomes?};
    \node[proc, below left=1.6cm and 0.8cm of d2](assign) {parseAssign()};
    \node[proc, below right=1.6cm and 0.8cm of d2](call)  {parseCall()};

    \draw[arr] (d1)   -- node[lbl, above] {no}  (kw);
    \draw[arr] (d1)   -- node[lbl, right] {yes} (scan);
    \draw[arr] (scan) -- (d2);
    \draw[arr] (d2.south) -- ++(0,-0.4) -| node[lbl, pos=0.25, above] {yes} (assign.north);
    \draw[arr] (d2.south) -- ++(0,-0.4) -| node[lbl, pos=0.25, above] {no}  (call.north);
\end{tikzpicture}
...
\begin{tikzpicture}[
    every node/.style={font=\small},
    proc/.style={rectangle, draw, fill=codebg, minimum width=3cm,
                 minimum height=0.8cm, rounded corners=2pt},
    dec/.style={diamond, draw, fill=yellow!10, aspect=2.6, inner sep=2pt,
                minimum width=2.6cm, minimum height=1cm},
    term/.style={rounded rectangle, draw, fill=black!8,
                 minimum width=2.8cm, minimum height=0.8cm},
    arr/.style={-{Stealth[length=2.5mm]}, thick},
    lbl/.style={font=\scriptsize, fill=white, inner sep=1pt}
]
    % Left decision column — all at x=0
    \node[term] (start)  at (0,   9.5) {START};
    \node[term] (init)   at (0,   8.4) {parseProgram()};
    \node[dec]  (dkw)    at (0,   6.8) {keyword?};
    \node[dec]  (dident) at (0,   5.2) {ident?};
    \node[dec]  (deps)   at (0,   3.6) {$\varepsilon$?};
    \node[term] (end)    at (0,   2.0) {EOFILE};

    % Action column — x=5.8, y aligned with decision nodes
    \node[proc] (rkw)  at (5.8, 6.8) {dispatch by keyword};
    \node[proc] (rdis) at (5.8, 5.2) {scan lookahead};
    \node[proc] (rsk)  at (5.8, 3.6) {empty stmt};

    % Sub-action column — x=11.0, y between rkw and rsk
    \node[proc] (rassign) at (11.0, 6.0) {parseAssign()};
    \node[proc] (rcall)   at (11.0, 4.4) {parseCall()};

    % Sink — x=15.0, y same as rkw so rkw->push is a straight horizontal
    \node[proc] (push) at (15.0, 6.8) {append child};

    % Vertical spine
    \draw[arr] (start) -- (init);
    \draw[arr] (init.south)   -- (dkw.north);
    \draw[arr] (dkw.south)    -- node[lbl,right]{no}  (dident.north);
    \draw[arr] (dident.south) -- node[lbl,right]{no}  (deps.north);
    \draw[arr] (deps.south)   -- node[lbl,right]{no}  (end.north);

    % Horizontal branches to action column
    \draw[arr] (dkw.east)    -- node[lbl,above]{yes} (rkw.west);
    \draw[arr] (dident.east) -- node[lbl,above]{yes} (rdis.west);
    \draw[arr] (deps.east)   -- node[lbl,above]{yes} (rsk.west);

    % rkw goes straight right to push (same y=6.8, no crossing)
    \draw[arr] (rkw.east) -- node[lbl,above]{keyword handler} (push.west);

    % rdis forks via a junction coordinate to rassign (up) and rcall (down)
    \coordinate (jct) at ($(rdis.east)+(0.5,0)$);
    \draw (rdis.east) -- (jct);
    \draw[arr] (jct) |- node[lbl,pos=0.65,above]{becomes assign} (rassign.west);
    \draw[arr] (jct) |- node[lbl,pos=0.65,below]{becomes call}   (rcall.west);

    % rassign and rcall merge into push via -| (go right then up to push.south)
    \draw[arr] (rassign.east) -| (push.south);
    \draw[arr] (rcall.east)   -| (push.south);

    % rsk routes to push below all other nodes
    \draw[arr] (rsk.east) -- ++(1.5,0) |- (push.south);

    % Loop back: push top goes up then left back to dkw top
    \draw[arr, dashed] (push.north) -- ++(0,0.6) -| (dkw.north);
\end{tikzpicture}}
...
\begin{tikzpicture}[
    ...
    level 4/.style={sibling distance=2.4cm},
  ...
  child {node {\texttt{<simple-expression>}}
    child {node {\texttt{<term>}}
      child {node {\texttt{<factor>}}
        child {node {\texttt{<variable>}}
          child {node {ident(a)}}
        }
      }
    }
    child {node {\texttt{<additive-operator>}}
      child {node {plus(+)}}
    }
    child {node {\texttt{<term>}}
      child {node {\texttt{<factor>}}
      ...
      child {node {\texttt{<multiplicative-operator>}}
        child {node {times(*)}}
      }
      child {node {\texttt{<factor>}}
        child {node {intcon(2)}}
...
\end{tikzpicture}}
\caption{Parse tree untuk ekspresi \texttt{a + b * 2}. Node \texttt{<variable>}
membungkus setiap \texttt{ident} sebagai hasil revisi struktur flat
\texttt{<variable>}/\texttt{<component-variable>}.}
\label{fig:tree-expr}
\end{figure}
...
pasangan file input-output dengan format nama \texttt{input-\{n\}.txt} dan
\texttt{output-\{n\}.txt}. Sepuluh test cases disusun untuk meliputi case golden-path,
fitur tipe data lanjutan, kontrol alur, prosedur/fungsi, operator boolean,
pemanggilan fungsi bersarang, tipe enumerasi, hingga skenario error.

\subsection{Lokasi File Pengujian}

\begin{itemize}
    \item file input: \texttt{test/milestone-2/input-1.txt} s.d. \texttt{test/milestone-2/input-10.txt}
    \item file output: \texttt{test/milestone-2/output/output-1.txt} s.d. \texttt{test/milestone-2/output/output-10.txt}
\end{itemize}
...
8 & Operator boolean           & Ekspresi kondisi dengan \texttt{and}, \texttt{or}, \texttt{not} pada \texttt{if} dan \texttt{while}. \\
9 & Pemanggilan fungsi bersarang & Argumen fungsi berupa pemanggilan fungsi lain (nested calls). \\
10 & Tipe enumerasi \& \texttt{case} & Deklarasi tipe enumerasi dan \texttt{case} dengan daftar konstanta. \\
\bottomrule
\end{tabularx}
\caption{Ringkasan sepuluh kasus uji Milestone~2.}
\end{table}

\subsection{Hasil Uji}

Setiap kasus uji disertai listing kode sumber Arion (\textit{input}) dan
parse tree atau pesan error yang dihasilkan program (\textit{output}).
Output lengkap tersimpan pada \texttt{test/milestone-2/output/}; output
dengan panjang lebih dari 40 baris hanya ditampilkan sebagian.

% ---- Kasus 1 ----
\subsubsection*{Kasus 1 - Golden-path program minimal}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-1.txt}}
]{../../test/milestone-2/input-1.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    caption={\texttt{test/milestone-2/output/output-1.txt}}
]{../../test/milestone-2/output/output-1.txt}

% ---- Kasus 2 ----
\subsubsection*{Kasus 2 - Deklarasi \texttt{const}, \texttt{type}, dan array bertingkat}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-2.txt}}
]{../../test/milestone-2/input-2.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-2.txt} (40 baris pertama dari \the\value{lstnumber} total)}
]{../../test/milestone-2/output/output-2.txt}

% -- Kasus 3 --
\subsubsection*{Kasus 3 - Subprogram: prosedur dan fungsi}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-3.txt}}
]{../../test/milestone-2/input-3.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-3.txt} (40 baris pertama)}
]{../../test/milestone-2/output/output-3.txt}

% -- Kasus 4 --
\subsubsection*{Kasus 4 - Kontrol alur: \texttt{if}, \texttt{case}, \texttt{while}, \texttt{repeat}, \texttt{for}}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-4.txt}}
]{../../test/milestone-2/input-4.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-4.txt} (40 baris pertama)}
]{../../test/milestone-2/output/output-4.txt}

% -- Kasus 5 --
\subsubsection*{Kasus 5 - \texttt{record}, field access, dan akses array dengan indeks variabel}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-5.txt}}
]{../../test/milestone-2/input-5.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-5.txt} (40 baris pertama)}
]{../../test/milestone-2/output/output-5.txt}

% -- Kasus 6 --
\subsubsection*{Kasus 6 - Error: urutan deklarasi tidak valid}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-6.txt}}
]{../../test/milestone-2/input-6.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/output/output-6.txt}}
]{../../test/milestone-2/output/output-6.txt}

% -- Kasus 7 --
\subsubsection*{Kasus 7 - Error: token leksikal tidak valid (\texttt{=} bukan \texttt{==})}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-7.txt}}
]{../../test/milestone-2/input-7.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/output/output-7.txt}}
]{../../test/milestone-2/output/output-7.txt}

% -- Kasus 8 --
\subsubsection*{Kasus 8 - Operator boolean: \texttt{and}, \texttt{or}, \texttt{not}}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-8.txt}}
]{../../test/milestone-2/input-8.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-8.txt} (40 baris pertama)}
]{../../test/milestone-2/output/output-8.txt}

% -- Kasus 9 --
\subsubsection*{Kasus 9 - Pemanggilan fungsi bersarang (\textit{nested calls})}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-9.txt}}
]{../../test/milestone-2/input-9.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    firstline=1, lastline=40,
    caption={\texttt{test/milestone-2/output/output-9.txt} (40 baris pertama)}
]{../../test/milestone-2/output/output-9.txt}

% -- Kasus 10 --
\subsubsection*{Kasus 10 - Tipe enumerasi dan \texttt{case} dengan daftar konstanta}

\lstinputlisting[language={}, numbers=none, basicstyle=\ttfamily\small,
    caption={\texttt{test/milestone-2/input-10.txt}}
]{../../test/milestone-2/input-10.txt}

\lstinputlisting[language={}, numbers=left, basicstyle=\ttfamily\scriptsize,
    caption={\texttt{test/milestone-2/output/output-10.txt}}
]{../../test/milestone-2/output/output-10.txt}
